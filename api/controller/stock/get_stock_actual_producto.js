...
export async function getStockActualbyProducto({ db, req }) {
    // taer did_producto y did_producto_combinacion
    const { did_producto, did_producto_combinacion } = req.params;
    const didProducto = Number(did_producto);
    const didProductoCombinacion = did_producto_combinacion ? Number(did_producto_combinacion) : null;


    // si did_combinacion es null o no viene, traer todo el producto, si viene solo traigo esa combinacion
    ...
    */
    ...
    if (didProductoCombinacion !== null) {
        const stockCombinacion = await LightdataORM.select({
            db,
            table: "stock_producto",
            where: { did_producto_combinacion: didProductoCombinacion },
        });
    }
    // mapear para 
