...
export async function createPedido(dbConnection, req) {
    ...
    const rowsDetalle = items
        ...
        .map((p) => {
            ...
            return {
                did_pedido: didPedido,                     // FK correcto
    ...
    if (rowsDetalle.length > 0) {
        await LightdataORM.insert({
            dbConnection,
            table: "pedido_productos", // ajustá si tu tabla es "pedidos_productos"
            quien: userId,
            data: rowsDetalle,
        });
    }

    // 3) Dirección: "pedido_direccion" (sin 'did' externo, sólo FK did_pedido)
    ...
    if (direccion && typeof direccion === "object") {
        ...
        const [insertedDidDireccion] = await LightdataORM.insert({
            dbConnection,
            table: "pedidos_ordenes_direcciones_destino", // ajustá al nombre real
            quien: userId,
            data: rowDireccion,
        });
