...
class ProductO1 {
  ...
  async traerProductos(connection, data = {}) {
    try {
      ...
      const results = await executeQuery(connection, query, [...valores, cantidad, offset], true)
  ...
  async traerProductoId(connection, id) {
    try {
      const query = `
        ...
        LEFT JOIN productos_ecommerce AS pe 
      ...
      for (const row of rows) {
        ...
        if (row.didEcommerce && !setEcommerce.has(row.didEcommerce)) {
          ...
          producto.ecommerce.push({
            ...
            variante: row.varianteEcommerce ? JSON.parse(row.varianteEcommerce) : null,
