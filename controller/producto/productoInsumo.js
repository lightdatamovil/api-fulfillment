...
class ProductoInsumo {
  ...
  async checkAndUpdateDidProducto(connection) {
    try {
      const checkDidProductoQuery =
        "SELECT did FROM productos_insumos WHERE didProducto = ? AND didInsumo = ? AND elim = 0 AND superado = 0";
      ...
      if (results.length > 0) {
        const updateQuery =
          "UPDATE productos_insumos SET superado = 1 WHERE did = ?";
  ...
  async createNewRecord(connection) {
    try {
      ...
      const insertQuery = `INSERT INTO productos_insumos (${filteredColumns.join(
        ", "
  ...
  async createNewRecord2(connection, did) {
    try {
      ...
      const insertQuery = `INSERT INTO productos_variantes (${filteredColumns.join(
        ", "
      )}) VALUES (${filteredColumns.map(() => "?").join(", ")})`;
      const insertResult = await executeQuery(connection, insertQuery, values);
